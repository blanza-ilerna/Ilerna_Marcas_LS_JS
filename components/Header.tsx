...
import { Box, Code2, LayoutTemplate, Calculator, GitMerge, BookOpen, BrainCircuit } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ activeTab, onTabChange }) => {
  const tabs = [
    { id: 'history', label: 'Historia y ES6', icon: BookOpen },
    { id: 'basics', label: 'Variables y Tipos', icon: Box },
    { id: 'operators', label: 'Operadores', icon: Calculator },
    { id: 'loops', label: 'Lógica y Bucles', icon: GitMerge },
    { id: 'functions', label: 'Funciones', icon: Code2 },
    { id: 'dom', label: 'DOM y Eventos', icon: LayoutTemplate },
    { id: 'quiz', label: 'Test Final', icon: BrainCircuit },
  ];
    ...
    <header className="mb-8">
      <div className="text-center mb-8">
        <div className="mb-6 flex justify-center">
          <svg width="113" height="23" viewBox="0 0 113 23" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ height: '50px', width: 'auto' }}>
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-ilerna-cyan mb-3">JavaScript Moderno</h1>
        <p className="text-lg text-ilerna-dark font-normal">Curso de Desarrollo de Aplicaciones Web</p>
      </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 rounded-t-xl transition-all duration-300 font-bold text-sm md:text-base
                ${isActive 
                  ? 'bg-ilerna-cyan text-white shadow-lg translate-y-[1px]' 
                  : 'bg-white text-gray-500 hover:bg-gray-100 hover:text-ilerna-cyan'
                }`}
            >
              <Icon size={18} />
              {tab.label}
